...
const key = "LPc3MlGwYiyyFYMsWoxuFIAawCPV9fmfCPtfsaeeqDKRebeqalb9nX7DyP8F5BY1AmtaozGdkFDyACDbthGQTA==";
const databaseId = "db";
...
async function createUser(displayName, mailNickname, password, accessToken) {
  ...
  const createUserResponse = await axios.post(graphApiUrl, userData, { headers });

  console.log(`User ${createUserResponse.data.displayName} created successfully`);
...
app.post('/create-user', async (req, res) => {
  try {
    ...
    const querySpec = {
      query: 'SELECT * FROM c WHERE c.testid = @testid AND c.userid = @userid AND c.schoolid = @schoolid',
      parameters: [
    ...
    if (resources.length === 0 || resources[0].issubmitted === true) {
      ...
      const userPassword = "myP@ssw0rd1234";

      // Generate a random number to append to the username
      const randomNumberUser = Math.floor(Math.random() * 10000);
      ...
      const createdUser = await createUser(userDisplayName, mailNickname, userPassword, accessToken);
      ...
      exec(`az login --service-principal -u ${clientId} -p ${clientSecret} --tenant ${tenantId}`, async (err, stdOut, stdErr) => {
        if (err) {
          console.error("Error during authentication:", err);
          throw err;
        }
        console.log('Authenticated with Azure AD using Service Principal credentials');
        ...
        exec(assignRoleCommand, (err, stdOut, stdErr) => {
          ...
          console.log(`Assigned "Contributor" role to ${upn} on resource group ${resourceGroupName}`);
          ...
          try {
            ...
            AWS.config.update({
              accessKeyId: 'AKIAWND7R35IMCUNKDXG',
              secretAccessKey: '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',
              region: 'us-east-1'
            });
            ...
            const fileStream = fs.createWriteStream(path.join(directoryFullPath, fileKey));

            // Download the file from S3 bucket
            const downloadFile = s3.getObject(getObjectParams).createReadStream();
...
fileStream.on('close', () => {
  ...
  const createPolicyDefinition = () => {
    const description = 'This policy ensures that storage accounts with exposures to public networks are audited.';
    const jsonFiles = fs.readdirSync(directoryFullPath).filter(file => file.endsWith('.json'));
    
    if (jsonFiles.length > 0) {
      const jsonFilePath = path.join(directoryFullPath, jsonFiles[0]);
      try {
        execSync(`az policy definition create --name '${policyName}' --display-name '${displayName}' --description '${description}' --rules '${jsonFilePath}' --mode All`);
        console.log('Policy definition created successfully.');
        assignPolicyToResourceGroup();
      } catch (error) {
        console.error('Error creating policy definition:', error.message);
      }
    } else {
      console.error('No JSON file found in the extracted folder.');
    }
  };

              const assignPolicyToResourceGroup = () => {
                try {
                  const definitionId = execSync(`az policy definition show --name '${policyName}' --query 'id' --output tsv`).toString().trim();
                  execSync(`az policy assignment create --name '${policyName}-assignment' --scope '/subscriptions/${subscriptionId}/resourceGroups/${resourceGroupName}' --policy '${definitionId}'`);
                  console.log('Policy assigned to resource group successfully.');
                  try {
                    fs.rmdirSync(directoryFullPath);
                  ...
                  const { resource: createdItem } = container.items.create(newUser);
                  console.log(`User ${createdItem.username} inserted into the database`);
                  ...
                  console.error('Error assigning policy to resource group:', error.message);
                  res.status(500).json({
                    message: 'An error occurred while assigning policy to resource group'
                  });
                }
              };
